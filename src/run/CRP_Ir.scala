...
object CRP_Ir {
  def main(args: Array[String]): Unit = {
    ...
    val file = if (args.nonEmpty)
      s"results/crp/crp-ir-caserta-(${args(0)},${args(1)},${args(2)},${args(3)}).csv"
    else
      "results/crp/crp-ir-caserta.csv"
    ...
    for ((h, s, start, end) <- HS) {
      for (i <- start to end) {
        ...
        val crp_ir = ins.toCRP_Ir(incumbent = sol.snaps)
        val res = crp_ir.solve()
